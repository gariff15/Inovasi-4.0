...
if st.session_state.page == 1:
    ...
    for i, q in enumerate(stress_questions):
        # Use radio box without default selected answer (unticked)
        answer = st.radio(q, options, index=-1, key=f"stress_q{i}")
        st.session_state.stress_answers[i] = answer
    if st.button("Next ➡️", type="primary"):
        if None in st.session_state.stress_answers or any(ans is None for ans in st.session_state.stress_answers):
...
# --- PAGE 2: Performance Assessment ---
    ...
    for i, q in enumerate(performance_questions):
        answer = st.radio(q, options, index=-1, key=f"perf_q{i}")
    ...
    with col2:
        if st.button("Next ➡️"):
            if None in st.session_state.performance_answers or any(ans is None for ans in st.session_state.performance_answers):
                st.warning("Please answer all performance questions before proceeding.")
            else:
                st.session_state.page = 3
                # Reset reveal flags to start fresh results display
                st.session_state.stress_revealed = False
                st.session_state.perf_revealed = False
                st.rerun()
...
# --- PAGE 3: Results with centered 'Assess' buttons in boxes ---
    ...
    with col1:
        ...
        if not st.session_state.stress_revealed:
            # Center 'Assess' button inside box
